...
curl --cookie-jar cookies.txt -H 'Content-Type: application/json' -d '{"email":"user@acme.com","password":"123456"}' -X POST http://localhost:8080/rest/user/login
###TOKEN=`cat token.txt`
curl -b cookies.txt http://localhost:8080/profile --cookie-jar cookies.txt
